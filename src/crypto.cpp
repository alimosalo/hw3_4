...
std::string crypto::string_to_hex(const std::string& input)
{
...
std::string crypto::sha256_tmp(std::string s)
{
  unsigned char hash[SHA256_DIGEST_LENGTH];
  SHA256((unsigned char*)s.c_str(), s.length(), hash);
  std::cout << hash << std::endl;
  std::string ret{reinterpret_cast<char*>(hash)};
  ret = ret.substr(0, SHA256_DIGEST_LENGTH);
  return ret;
}
